...
//                    makeFastRotations(new Shape(T_STR)),
    ...
    public Shape rotateCounterclockwise() {
        ...
        for (int i = 0; i < points.length; i++) {
            Point rotateShape = points[i];
            int x_new = getHeight() - rotateShape.y - 1;
            int y_new = rotateShape.x;
            Point rotatedShape = new Point(x_new, y_new);
            rotated_shape[i] = rotatedShape;
        }
        return new Shape(rotated_shape);
    }
